...
| System config | `nix/host.nix` | `starship`; brew formulas `autoconf`, `herdr`, and the C++ toolchain `cmake`, `ninja`, `ccache`, `llvm` (keg-only); casks `wezterm`, `amethyst`, `opensuperwhisper`; macOS defaults (incl. OpenSuperWhisper Cmd+` record hotkey) |
...
| C++ lint tools | `nix/home/darwin.nix` `home.file` | links to Homebrew LLVM's `clang-format`, `clang-tidy`, `run-clang-tidy`, `clang-apply-replacements`, `git-clang-format` in `~/.local/share/ic/llvm-tools`, on `PATH` without shadowing Apple `clang` |
...
With the agents repo cloned, the personal-layer checks also cover every Claude subagent and rule link (and dangling ones), every hook script that `claude/settings.json` runs around Bash and edit calls (the guard and the post-edit lint feedback), and `bin/build-manuals --check`, so a stale or hand-edited manual is caught on the machine and not only in CI.
