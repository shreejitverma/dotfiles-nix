...
- an agents repo lacking `GROK.md` and `grok/agents`: each FAIL names the missing source rather than offering `run: ic-link`, which would be a no-op there, and the links that removal leaves dangling are all named in their own FAIL line, since the dangling scan does not depend on how many definitions the repo still has
