...
It covers: a behind fork fast-forwarded, pushed, and reinstalled; an up-to-date fork whose install command must not re-run and whose stale local `user.email`/`user.name` overrides are stripped; a diverged fork reported and left byte-for-byte untouched (locally, on the fork remote, and by `gh repo sync`, which must not be called for it); dirty-tree, wrong-branch, uncloned, and `sync: false` entries; 30-day log rotation; a missing `upstream` remote failing the repo rather than reading as up to date; failure notifications outranking diverged ones; a host with no manifest exiting 0 quietly; `--dry-run` reporting drift while mutating nothing; a stray global identity failing the repo before any sync; and `ic-workflow.zsh` sourcing the generated fleet aliases file when present and sourcing cleanly when absent.

The fleet manifest, `gh` behaviour, and desktop notifications are the suite's stub boundary; the launchd schedule and systemd timer that trigger the script are nix module config, asserted by `nix eval` rather than by this suite.
