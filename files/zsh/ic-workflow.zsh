...
# compact-adviser (a fleet plugin wired into Claude Code and Grok) reads its
# TypeSafe key from TYPESAFE_API_KEY. The key lives in the login Keychain and
# reaches these two commands only, per invocation: a global export would also
# opt firstmate into typed dispatch, which presence-gates on the same variable
# (firstmate bin/fm-dispatch-resolve.sh), and saving it through Claude's
# /compact-adviser menu would write it into the git-tracked
# ~/github/agents/claude/settings.json. `command claude` and script launches
# bypass these functions, so the first mate `fm` starts and every unattended
# agent run get no key. Store or rotate it once with:
#   security add-generic-password -U -a "$USER" -s typesafe-api-key -w
if [ "$IC_PLATFORM" = darwin ]; then
  _ic_with_typesafe_key() {
    local key
    key=$(security find-generic-password -a "$USER" -s typesafe-api-key -w 2>/dev/null)
    if [ -n "$key" ]; then
      TYPESAFE_API_KEY=$key command "$@"
    else
      command "$@"
    fi
  }
  claude() { _ic_with_typesafe_key claude "$@"; }
  grok() { _ic_with_typesafe_key grok "$@"; }
fi
