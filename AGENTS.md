...
- Each platform entry module (`nix/user.nix`, `nix/linux-user.nix`, `nix/wsl-user.nix`) declares its own one-line `dotfilesDir` literal, and each carries an assertion that the literal equals `config.ic.dotfilesDir`. The literal is not decorative: `setup/mac.sh`, `setup/linux.sh`, `files/bin/ic-link`, and `files/bin/ic-doctor` parse it textually. Keep it on one line in the existing shape, or those consumers silently stop finding it and their guards degrade to no-ops.
- `nix/home/dotfiles.nix` is the single definition of `dotfilesDir` that the four layers read; it is `readOnly`, so a second definition is a build error rather than a silent divergence. Do not reintroduce a per-layer `let dotfilesDir = "${config.home.homeDirectory}/..."`, and do not fold the entry-module literals into it: the entry modules keep theirs because the shell-side guards parse them, and the assertion is what ties the two together.
