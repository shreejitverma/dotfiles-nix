...
- The `~/AGENTS.md` fallback to Claude's manual in `files/bin/ic-link` is gated on the private agents repo being absent, not on the tool-neutral `AGENTS.md` being missing. A cloned but ungenerated repo must warn and leave the link alone, because silently handing Codex Claude-only rules is the exact fault the per-tool manuals exist to remove, and `ic-doctor` FAILs that state rather than accepting it. Covered by the `agents-md-absent` and `agents-md-no-repo` scenarios in `tests/ic_link_test.sh`.
